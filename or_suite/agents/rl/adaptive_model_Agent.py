...
class AdaptiveModelBasedDiscretization(Agent):
    ...
    def update_obs(self, obs, action, reward, newObs, timestep, info):
        ...
        if timestep != self.epLen - 1:
            ...
            # print('Update estimates and leaves!')
            # print(np.asarray(next_tree.state_leaves).shape)
            # print(next_tree.state_leaves)
            # print(next_tree.state_leaves[0])
            # print(newObs)
            # print(np.max(np.abs(np.asarray(next_tree.state_leaves) - newObs)))
            new_obs_loc = np.argmin(np.max(np.abs(np.asarray(next_tree.state_leaves) - newObs)))
            # print(new_obs_loc)
    ...
    def update_policy(self, k):
        ...
        if self.flag: # flag corresponds to one step vs full step updates
            for h in np.arange(self.epLen-1,-1,-1):
                ...
                for node in tree.leaves:
                    ...
                    index = 0
                    for state_val in tree.state_leaves:
                        _, qMax = tree.get_active_ball(np.asarray(state_val))
                        tree.vEst[index] = min(qMax, self.epLen, tree.vEst[index])
                        index += 1
